...
import TemplateBox1 from "../molecules/templateBox1";
...
const getAudioEmbedUrl = (url: string): { type: 'direct' | 'embed'; url: string } | null => {
  try {
    // 🔹 Caso 1: archivo directo (.mp3, .wav, .ogg)
...
export default function TopicTemplate1() {
  const router = useRouter();
  const [layout, setLayout] = useState<"grid1">("grid1");
  ...
  const handleSaveTemplate = async () => {
   ...
   return (
  <main
   className="bg-[#C9DDDC] min-h-screen w-full flex flex-col items-center justify-start p-8 md:p-12 overflow-auto"
   ...
   <div className="flex flex-col items-center gap-3 md:gap-4 w-full max-w-7xl flex-1 min-h-0">
    ...
    <div className="flex-1 w-full min-h-0">
     ...
     >
        {topicData ? (
    renderPreviewBlock({
      type: topicData.videos?.length
        ? "video"
        : topicData.images?.length
        ? "image"
        : topicData.audios?.length
        ? "audio"
        : topicData.presentations?.length
        ? "presentation"
        : "subtitles",
      data:
        topicData.videos ||
        topicData.images ||
        topicData.audios ||
        topicData.presentations ||
        topicData.subtitles ||
        [],
    })
  ) : (
    <span className="text-gray-400">Cargando contenido...</span>
  )}
     </TemplateBox1>
    </div>
